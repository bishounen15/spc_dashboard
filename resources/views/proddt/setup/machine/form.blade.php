@extends('layouts.app')
@section('content')
<form method="POST" action="{{ $modify == 1 ? '/proddt/setup/machine/' . $id : '/proddt/setup/machine' }}" id="StationForm">
    @if($modify == 1)
    <input type="hidden" name="_method" value="put" />
    @endif
    @csrf 
    <div class="container">
        <h3>Machine {{ $modify == 1 ? 'Update' : 'Creation' }}</h3>
        <div class="card">
            ...
            <div class="card-footer">
                <div class="form-row">
                    <div class="form-group col-sm-6">
                        <input type="submit" class="btn btn-success" name="save" id="save" value="{{ $modify == 1 ? 'Update' : 'Create' }} Machine" style="width: 200px;">
                    </div>
                    <div class="form-group col-sm-6 text-right">
                        <a href="/proddt/setup/machine" role="button" class="btn btn-danger" style="width: 200px;">Cancel</a>
                    </div>
                </div>
            </div>
        </div>
