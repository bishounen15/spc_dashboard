...
class ReportsController extends Controller
{
    ...
    function moduleInfo(Request $request, $sdate = null, $edate = null) {
        ...
        $modInfo = DB::connection("trina")->select("SELECT A.Module_ID, B.OrderID, A.WorkOrder_ID, A.WorkOrder_vertion, IFNULL(L.Product_ID, B.Product_ID) AS Product_ID, C.Product_Type, A.Grade_of_Cell_Power, A.Cell_Power, A.Cell_Color, Q.EFF, A.Module_Grade, A.EL_Grade, A.Status, L.Carton_No, O.Title, O.TEST_DATETIME, DATE_ADD(L.Packing_Date,INTERVAL CASE WHEN L.Packing_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) as Packing_Date, L.State as 'PackingState', M.Container_no, DATE_ADD(M.buildcabtime,INTERVAL CASE WHEN M.buildcabtime < '2019-04-26' THEN 15 ELSE 0 END HOUR) as 'ContainerDate' FROM omes.rt_wo_mid A INNER JOIN omes.df_wo_mat B ON A.WorkOrder_ID = B.WorkOrder_ID AND A.WorkOrder_vertion = B.WorkOrder_vertion INNER JOIN omes.df_pid_type_mapping C ON B.Product_ID = C.Q1_ID INNER JOIN df_module_cell_power Q ON A.Grade_of_Cell_Power = Q.Code AND B.Cell_Suppliers = Q.factory AND A.Cell_Power = Q.power left join omes.rt_mid_packing L on A.Module_ID = L.Module_ID AND L.State IN ('Packed','Wait') left join omes.rt_mid_buildcontainer M on L.Carton_No = M.Carton_no left join omes.rt_mid_flash O on A.Module_ID = O.Module_ID left join omes.rt_module_elresult P on A.Module_ID = P.MODULEID WHERE A.Module_ID LIKE 'S98%' AND (A.WorkOrder_ID LIKE 'S19%' OR A.WorkOrder_ID LIKE 'S2%') AND DATE_ADD(A.Create_Date,INTERVAL CASE WHEN A.Create_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) BETWEEN ? AND ? ".$cond.$scond.$filtercond." ORDER BY A.WorkOrder_ID, A.Module_ID",[$start,$end]);
    ...
    function containerInfo(Request $request, $sdate = null, $edate = null, $shipped = null) {
        ...
        $container = DB::connection("trina")->select("SELECT '' as `Contract no`, a.Container_No as `Batch No`, a.Carton_No as 'Carton No', a.WorkOrder_ID as 'Workorder ID', a.Module_ID as 'Module ID', a.Product_ID as 'Product ID', a.Product_Type as 'Product Type', f.po_number as 'Purchase Order', '' as 'Country Of Original', b.Cell_Suppliers as 'Cell Suppliers', '' as 'CONTAINER No', '' as 'SEAL', '' as 'BOL', '' as 'Ship destination', b.Layout_QTY_of_Cell as 'Cells Per Panel', DATE_ADD(c.Create_Date,INTERVAL CASE WHEN c.Create_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) as 'Production Date', b.Cell_MID as 'cell No', a.Module_Grade as 'MODULE GRADE', CASE a.WorkOrder_ID WHEN a.ORG_WO THEN '' ELSE a.ORG_WO END as 'Orig_WO', CASE a.WorkOrder_ID WHEN a.ORG_WO THEN '' ELSE g.Cell_MID END AS 'Orig_Cell_PartNo', CASE a.WorkOrder_ID WHEN a.ORG_WO THEN '' ELSE g.Cell_Suppliers END AS 'Orig_Cell_Suppliers' FROM omes.rt_mid_packing a inner join omes.df_wo_mat b on a.WorkOrder_ID = b.WorkOrder_ID AND a.WorkOrder_vertion = b.WorkOrder_vertion inner join omes.rt_wo_mid c on a.Module_ID = c.Module_ID INNER JOIN omes.df_pid_type_mapping d ON b.Product_ID = d.Q1_ID LEFT JOIN omes.rt_mid_cellsizing e ON a.Module_ID = e.Module_ID LEFT JOIN solarph.po_data f ON e.CellBarCode = f.cell_lot left join omes.df_wo_mat g on a.ORG_WO = g.WorkOrder_ID and a.WorkOrder_vertion = g.WorkOrder_vertion where a.Module_ID like 'S98%' AND a.State = 'Packed' AND DATE_ADD(a.Packing_Date,INTERVAL CASE WHEN a.Packing_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) BETWEEN ? AND ?" . $cond . $filtercond . " ORDER BY a.Carton_No, a.Packing_Date",[$start,$end]);
