...
class ReportsController extends Controller
{
    ...
    function containerInfo(Request $request, $sdate = null, $edate = null, $shipped = null) {
        ...
        $container = DB::connection("trina")->select("SELECT '' as `Contract no`, a.Container_No as `Batch No`, a.Carton_No as 'Carton No', a.WorkOrder_ID as 'Workorder ID', a.Module_ID as 'Module ID', a.Product_ID as 'Product ID', a.Product_Type as 'Product Type', f.po_number as 'Purchase Order', '' as 'Country Of Original', b.Cell_Suppliers as 'Cell Suppliers', '' as 'CONTAINER No', '' as 'SEAL', '' as 'BOL', '' as 'Ship destination', b.Layout_QTY_of_Cell as 'Cells Per Panel', DATE_ADD(c.Create_Date,INTERVAL CASE WHEN c.Create_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) as 'Production Date', b.Cell_MID as 'cell No', a.Module_Grade as 'MODULE GRADE', CASE a.WorkOrder_ID WHEN a.ORG_WO THEN a.WorkOrder_ID ELSE a.ORG_WO END as 'Orig. WO', CASE a.WorkOrder_ID WHEN a.ORG_WO THEN b.Cell_MID ELSE g.Cell_MID END AS 'Orig_Cell_PartNo', CASE a.WorkOrder_ID WHEN a.ORG_WO THEN b.Cell_Suppliers ELSE g.Cell_Suppliers END AS 'Orig. Cell Suppliers' FROM omes.rt_mid_packing a inner join omes.df_wo_mat b on a.WorkOrder_ID = b.WorkOrder_ID inner join omes.rt_wo_mid c on a.Module_ID = c.Module_ID INNER JOIN omes.df_pid_type_mapping d ON b.Product_ID = d.Q1_ID LEFT JOIN omes.rt_mid_cellsizing e ON a.Module_ID = e.Module_ID LEFT JOIN solarph.po_data f ON e.CellBarCode = f.cell_lot inner join omes.df_wo_mat g on a.ORG_WO = g.WorkOrder_ID where a.Module_ID like 'S98%' AND a.State = 'Packed' AND DATE_ADD(a.Packing_Date,INTERVAL CASE WHEN a.Packing_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) BETWEEN ? AND ?" . $cond . $filtercond . " ORDER BY a.Carton_No, a.Packing_Date",[$start,$end]);
