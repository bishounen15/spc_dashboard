...
class ReportsController extends Controller
{
    ...
    function ftd($sdate = null, $edate = null, $pack = null) {
        ...
        $dfield = ($pack == null || $pack == "false" ? "TEST_DATETIME" : "DATE_ADD(Packing_Date, INTERVAL 15 HOUR)");
        
        $ftd = DB::connection("trina")->select("SELECT IFNULL(g.Judgement,'-') as Judgement, a.WorkOrder_ID, a.Module_ID, d.Product_ID, REPLACE(e.Product_Type,'***',b.LOWERPOWER) AS Product_Type, IFNULL(f.Carton_No,'') as Carton_no, (DATE_ADD(Packing_Date, INTERVAL 15 HOUR)) as Packing_Date, IFNULL(f.Container_No,'') AS Container_No, a.TEST_DATETIME, a.TITLE, b.LOWERPOWER as Grade, a.PMAX, a.FF, a.VOC, a.ISC, a.VPM, a.IPM, a.RS, a.RSH, a.EnvTemp, a.SurfTemp, IFNULL(CONCAT('ftp://192.168.128.25/',SUBSTR(a.Module_ID,1,10),'/',MAX(c.FILEPATH)),'') AS FILEPATH FROM omes.rt_mid_flash a inner join omes.df_wo_mat d on a.WorkOrder_ID = d.WorkOrder_ID inner join omes.df_module_powersetting b on a.WorkOrder_ID = b.WorkOrder_id and d.Product_ID = b.Product_ID inner join omes.df_pid_type_mapping e on d.Product_ID = e.Q1_ID and (a.PMAX >= b.LOWERPOWER and a.PMAX < b.UPPERPOWER) LEFT JOIN omes.rt_modulepelresults c on a.Module_ID = c.MODULEID left join omes.rt_mid_packing f on a.Module_ID = f.Module_ID and f.State = 'Packed' left join solarph.oba g on a.Module_ID = g.Module_ID and f.Carton_No = g.Carton_no where a.Module_ID like 'S98%' AND ".$dfield." BETWEEN ? AND ? ".$cond." GROUP BY g.Judgement, a.WorkOrder_ID, a.Module_ID, d.Product_ID, e.Product_Type, f.Carton_No, f.Packing_Date, f.Container_No, a.TEST_DATETIME, a.TITLE, b.LOWERPOWER, a.PMAX, a.FF, a.VOC, a.ISC, a.VPM, a.IPM, a.RS, a.RSH, a.EnvTemp, a.SurfTemp ORDER BY ".$sort,[$start,$end]);
    ...
    function containerInfo($sdate = null, $edate = null) {
        ...
        $container = DB::connection("trina")->select("SELECT '' as `Contract no`, a.Container_No as `Batch No`, a.Carton_No as 'Carton No', a.WorkOrder_ID as 'Workorder ID', a.Module_ID as 'Module ID', a.Product_ID as 'Product ID', a.Product_Type as 'Product Type', '' as 'Purchase Order', '' as 'Country Of Original', b.Cell_Suppliers as 'Cell Suppliers', '' as 'CONTAINER No', '' as 'SEAL', '' as 'BOL', '' as 'Ship destination', b.Layout_QTY_of_Cell as 'Cells Per Panel', DATE_ADD(c.Create_Date,INTERVAL 15 HOUR) as 'Production Date', b.Cell_MID as 'cell No', a.Module_Grade as 'MODULE GRADE' FROM omes.rt_mid_packing a inner join omes.df_wo_mat b on a.WorkOrder_ID = b.WorkOrder_ID inner join omes.rt_wo_mid c on a.Module_ID = c.Module_ID where a.Module_ID like 'S98%' AND a.State = 'Packed' AND DATE_ADD(a.Packing_Date,INTERVAL 15 HOUR) BETWEEN ? AND ? ORDER BY a.Carton_No, a.Packing_Date",[$start,$end]);
