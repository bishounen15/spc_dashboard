...
class ReportsController extends Controller
{
    ...
    function moduleInfo($sdate = null, $edate = null) {
        ...
        $modInfo = DB::connection("trina")->select("SELECT A.Module_ID, B.OrderID, A.WorkOrder_ID, A.WorkOrder_vertion, B.Product_ID, C.Product_Type, A.Module_Grade, A.EL_Grade, A.Status, L.Carton_No, DATE_ADD(L.Packing_Date,INTERVAL CASE WHEN L.Packing_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) as Packing_Date, L.State as 'PackingState', M.Container_no, DATE_ADD(M.buildcabtime,INTERVAL CASE WHEN M.buildcabtime < '2019-04-26' THEN 15 ELSE 0 END HOUR) as 'ContainerDate' FROM omes.rt_wo_mid A INNER JOIN omes.df_wo_mat B ON A.WorkOrder_ID = B.WorkOrder_ID INNER JOIN omes.df_pid_type_mapping C ON B.Product_ID = C.Q1_ID left join omes.rt_mid_packing L on A.Module_ID = L.Module_ID AND L.State = 'Packed' left join omes.rt_mid_buildcontainer M on L.Carton_No = M.Carton_no left join omes.rt_mid_flash O on A.Module_ID = O.Module_ID left join omes.rt_module_elresult P on A.Module_ID = P.MODULEID WHERE A.Module_ID LIKE 'S98%' AND DATE_ADD(A.Create_Date,INTERVAL CASE WHEN A.Create_Date < '2019-04-26' THEN 15 ELSE 0 END HOUR) BETWEEN ? AND ? ORDER BY A.WorkOrder_ID, A.Module_ID",[$start,$end]);
